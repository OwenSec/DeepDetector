    ...
    for k=3:imageNumber
        ...
        if (oriLabel == imageCorrectLabel)
            ...
            if (advLabel ~= oriLabel)
                ...
                imageForEntropy = uint8(image);
                imageForEntropy = imageForEntropy(:, :, [3, 2, 1]);
                imageForEntropy = permute(imageForEntropy, [2, 1, 3]);
                advForEntropy = uint8(advX);
                advForEntropy = advForEntropy(:, :, [3, 2, 1]);
                advForEntropy = permute(advForEntropy, [2, 1, 3]);
                
                tic;
                oriEntropy = image2DEntropy55(imageForEntropy);
                if (oriEntropy < 8.5)
...
caffe.reset_all(); % reset caffe
    ...
    function processedIm = meanFilter(im)
        im = single(im);
        processedIm = im;
        for c=1:3
            for w=3:222
    ...
    function entropy = image2DEntropy55(im)
        im2 = meanFilter(im);
        R_entropy = single(0);
        G_entropy = single(0);
        B_entropy = single(0);
        ...
        for i=1:256
            for j=1:256
                pmf_R(i,j)=single(R_num(i,j))/(224*224);
                pmf_G(i,j)=single(G_num(i,j))/(224*224);
                pmf_B(i,j)=single(B_num(i,j))/(224*224);
            end
        end
        for i=1:256
            for j=1:256
