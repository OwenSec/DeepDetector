...
def episilon1(rootDir,true_class):
    ...
    for root, dirs, files in list_dirs: 
        ...
        for f in files: 
            ...
            if (ori_entropy<4):
                im3=scalarQuantization(original_data,128)
            elif (ori_entropy<5):
                im3=scalarQuantization(original_data,64)
            else: 
                im1=scalarQuantization(original_data,43)
                im2=crossMeanFilterOperations(im1,3,221,13)
            ...
            if (adv_entropy<4):
                im6=scalarQuantization(adversarial_data,128)
            elif (adv_entropy<5):
                im6=scalarQuantization(adversarial_data,64)
            else: #adv_entropy>9.50
                im4=scalarQuantization(adversarial_data,43)
                im5=crossMeanFilterOperations(im4,3,221,13)
            ...
            if (adv_filtered_class==adv_class):
                FN=FN+1
            print("disturbedFailure:", disturbedFailure," Total: ",Total, " TP: ",TP," FN: ",FN," FP: ",FP," TTP:",TTP)
...
#step is the size of quantization interval,epsilon defines the perturbation(set as 1.0 in [0,255] scale)
def episilo2(rootDir,true_class):
    list_dirs = os.walk(rootDir) 
    Total=0.0
    TP=0.0
    FP=0.0
    ...
    for root, dirs, files in list_dirs: 
        ...
        for f in files: 
            ...
            grad = compute_gradient(original_data, pred_class)
            delta = np.sign(grad)
            adversarial_data=add_matrix(original_data,delta,2.0)
            net.blobs['data'].data[...] = adversarial_data
            adv_out = net.forward()
            adv_class = adv_out['prob'][0].argmax()
            if pred_class != true_class:
...
#episilon = 1
episilon1('/home/ll/DeepDetector/TotalImages/Zebra',340)
episilon1('/home/ll/DeepDetector/TotalImages/Panda',388)
episilon1('/home/ll/DeepDetector/TotalImages/Cab',468)
...
#episilon = 2
episilon2('/home/ll/DeepDetector/TotalImages/Zebra',340)
episilon2('/home/ll/DeepDetector/TotalImages/Panda',388)
episilon2('/home/ll/DeepDetector/TotalImages/Cab',468)
