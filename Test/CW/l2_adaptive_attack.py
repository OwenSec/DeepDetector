...
#            print('original = ',model.predict(batch.reshape(1,28,28,1)))    
 #           print('label = ', batchlab)
...
#                   print(iteration,self.sess.run((self.loss,self.loss1,self.loss2)))
    ...
    def normalization(self, image_data):
        image_data[image_data<-0.5] = -0.5
        image_data[image_data>0.5] = 0.5
        
    def scalarQuantization(self, inputDigit, interval):
    ...
    def crossMeanFilterOperations(self, inputDigit, start, end, coefficient):
    ...
    def chooseCloserFilter(self, original_data,filter_data1,filter_data2): #detection filter
    ...
    def expandImage(self, image_data):
        image_data2 = np.array(image_data)
        image_data2 = (image_data2+0.5)*255
        return image_data2
        
    def mnistPredicate(self,img):
        imgTobePre = np.reshape(img, (1,28,28,1))
        preList = np.squeeze(self.tempmodel.model.predict(imgTobePre))
        return preList.argmax()
                
    def transformed(self, tempInput):
        self.normalization(tempInput)
        entropy = self.oneDEntropy(tempInput)
        if entropy < 4:
            inputFinal = self.scalarQuantization(tempInput,128)
        elif entropy < 5:
            inputFinal =self.scalarQuantization(tempInput, 64)
        else:
            inputAfterQ = self.scalarQuantization(tempInput,43)
            inputAfterQandF = self.crossMeanFilterOperations(inputAfterQ,3,25,13)
            inputFinal = self.chooseCloserFilter(tempInput, inputAfterQ, inputAfterQandF)
        return self. mnistPredicate(inputFinal)
